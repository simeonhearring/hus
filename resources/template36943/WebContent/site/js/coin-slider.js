(function($) {var params=new Array;
	...
	$.fn.coinslider=$.fn.CoinSlider=function(options){init=function(el){
			...
			$(el).css({background:'url('+images[el.id][0]+')',width:980,height:481,position:'relative',backgroundPosition:'top right'}).wrap("<div class='coin-slider' id='coin-slider-"+el.id+"' />");
			$('#'+el.id).append("<div class='cs-"+el.id+"' id='cs-"+el.id+"' style='width:980px;height:396px;right:0;top:0;position:absolute;background-position:0 0;'></div><div class='banner-container'></div>");
			...
			$('#coin-slider').append("<div id='cs-buttons-"+el.id+"' class='cs-buttons'></div>")
			...
			$('.banner').eq(imagePos[el.id]).css({bottom:-100}).animate({bottom:0},500)
		}
		...
		$.transition=function(el,direction){if(params[el.id].pause==true)return;
			...
			$('.banner').animate({bottom:-100},function(){appInterval[el.id]=setInterval(function(){$.appereance(el,0)})})
		};
		$.appereance=function(el,sid){
			if(squarePos[el.id]==1){clearInterval(appInterval[el.id]);return}
			$('#cs-'+el.id).css({opacity:0,'background-image':'url('+images[el.id][imagePos[el.id]]+')'}).stop().animate({opacity:1},1200,function(){$('.banner').eq(imagePos[el.id]).css({opacity:1,bottom:-100}).animate({bottom:0},500)});
			squarePos[el.id]++;
		};
	this.each (
		function(){init(this);}
